...
export default class BleModule{
    ...
    start(){
        BleManager.start({showAlert: false})
            .then( ()=>{
                this.checkState();
                ////console.loglog('Init the module success.');                
            }).catch(error=>{
                //console.loglog('Init the module fail.');
            });
    }
    ...
    scan() {
        return new Promise( (resolve, reject) =>{
            BleManager.scan([], 5, true)
                .then( () => {
                   // //console.loglog('Scan started');
                    resolve();
                }).catch( (err)=>{
	                //console.loglog('Scan started fail');
	                reject(err);
	            });
        });
    }
    ...
    stopScan() {
        BleManager.stopScan()
            .then(() => {
                //console.loglog('Scan stopped');
            }).catch((err)=>{
	            //console.loglog('Scan stopped fail',err);
	        });
    }
        ...
        return new Promise( (resolve, reject) =>{
            BleManager.getDiscoveredPeripherals([])
                .then((peripheralsArray) => {
                    ////console.loglog('Discovered peripherals: ', peripheralsArray);
    ...
    getUUID(peripheralInfo){       
       ...
       // //console.loglog('readServiceUUID',this.readServiceUUID);
        ////console.loglog('readCharacteristicUUID',this.readCharacteristicUUID);
       ////console.loglog('writeWithResponseServiceUUID',this.writeWithResponseServiceUUID);
       ////console.loglog('writeWithResponseCharacteristicUUID',this.writeWithResponseCharacteristicUUID);
       // //console.loglog('writeWithoutResponseServiceUUID',this.writeWithoutResponseServiceUUID);
        ////console.loglog('writeWithoutResponseCharacteristicUUID',this.writeWithoutResponseCharacteristicUUID);
        //console.loglog('nofityServiceUUID',this.nofityServiceUUID);
        //console.loglog('nofityCharacteristicUUID',this.nofityCharacteristicUUID);  
    }
    ...
    connect(id) {
        ...
        return new Promise( (resolve, reject) =>{
            BleManager.connect(id)
                .then(() => {
                    ////console.loglog('Connected success.');
                    return BleManager.retrieveServices(id);                    
                })
                .then((peripheralInfo)=>{
                    //console.loglog('Connected peripheralInfo: ', peripheralInfo);                    
                ...
                .catch(error=>{
                    //console.loglog('Connected error:',error);
                    this.isConnecting = false;   //当前蓝牙连接结束  
                    reject(error);
                });
        });
    ...
    disconnect() {
	    BleManager.disconnect(this.peripheralId)
		    .then( () => {
			    //console.loglog('Disconnected');
		    })
		    .catch( (error) => {
			    //console.loglog('Disconnected error:',error);
		    });      
    }
    ...
    startNotification(index = 0) {
        return new Promise( (resolve, reject) =>{
            ...
            BleManager.startNotification(this.peripheralId, this.nofityServiceUUID[index], this.nofityCharacteristicUUID[index])
                .then(() => {
                    console.log(this.nofityCharacteristicUUID);
                    //console.loglog('Notification started');
                    resolve();
                })
                .catch((error) => {
                    console.log(error);
                    //console.loglog("nihaommamamama_____")
                    //console.loglog(this.nofityServiceUUID[index]);
                    //console.loglog(this.nofityCharacteristicUUID[index]);
                    //console.loglog('Notification error:',error);
                    reject(error);
                });
        });
    }
    ...
    stopNotification(index = 0) { 
        return new Promise( (resolve, reject) =>{
            ...
            .then(() => {
                //console.loglog('stopNotification success!');
                 resolve();
            })
            .catch((error) => {
                //console.loglog('stopNotification error:',error);
                reject(error);
            });
        });
    }
	...
	write(data,index = 0) {
        ...
        return new Promise( (resolve, reject) =>{
            BleManager.write(this.peripheralId, this.writeWithResponseServiceUUID[index], this.writeWithResponseCharacteristicUUID[index], data)
                .then(() => {
                    //console.loglog('Write success: ',data.toString());
                    resolve();
                })
                .catch((error) => {
                    //console.loglog(this.peripheralId, this.writeWithResponseServiceUUID[index], this.writeWithResponseCharacteristicUUID[index], data)
                    //console.loglog('Write  failed: ',data);
                    reject(error);
                });
        });       
    }
    ...
    writeWithoutResponse(data,index = 0){
        return new Promise( (resolve, reject) =>{
            BleManager.writeWithoutResponse(this.peripheralId, this.writeWithoutResponseServiceUUID[index], this.writeWithoutResponseCharacteristicUUID[index], data)
                .then(() => {
                    //console.loglog('Write success: ',data);
                    resolve();
                })
                .catch((error) => {
                    //console.loglog('Write  failed: ',data);
    ...
    getConnectedPeripherals() {
        BleManager.getConnectedPeripherals([])
            .then((peripheralsArray) => {
                //console.loglog('Connected peripherals: ', peripheralsArray);
            }).catch(error=>{
        ...
        return new Promise( (resolve, reject) =>{
            BleManager.isPeripheralConnected(this.peripheralId, [])
                .then((isConnected) => {
                    resolve(isConnected);
                    if (isConnected) {                        
                        //console.loglog('Peripheral is connected!');
         
                    } else {
                        //console.loglog('Peripheral is NOT connected!');
    ...
    /** 
     * 打开蓝牙(Android only) 
     * Cr1eate the request to the user to activate the bluetooth 
     * */
	enableBluetooth() {
	    BleManager.enableBluetooth()
		    .then(() => {
			    //console.loglog('The bluetooh is already enabled or the user confirm');
		    })
		    .catch((error) => {
			    //console.loglog('The user refuse to enable bluetooth');
		    });
    }
    ...
    createBond(){
        BleManager.createBond(this.peripheralId)
            .then(() => {
                //console.loglog('createBond success or there is already an existing one');
            })
            .catch(() => {
                //console.loglog('fail to bond');
            })
    }
    ...
    getBondedPeripherals(){
        BleManager.getBondedPeripherals([])
            .then((bondedPeripheralsArray) => {
                ...
                //console.loglog('Bonded peripherals: ' + bondedPeripheralsArray);
            });
    }
